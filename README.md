...
```text
热点导入：Google Trends -> Google News RSS 相关新闻 -> 原站 metadata -> 自动生成 SEO 新闻页
...
HOT_NEWS_FETCH_METADATA=true
HOT_NEWS_METADATA_PER_TOPIC=4
HOT_NEWS_REQUIRE_DESCRIPTION=true
...
RSS 和 Google News 来源通常不会提供完整原文或稳定图片，所以系统默认导入标题、摘要、图片、发布时间、来源链接，并尝试从原文页读取 `og:image`、标题、描述和 canonical。热点词只作为发现入口，不再生成公开的关键词趋势简报页；热点导入可用 `HOT_NEWS_FETCH_METADATA` 开关控制是否抓取原文 metadata，用 `HOT_NEWS_METADATA_PER_TOPIC` 控制每个热点最多补抓几篇，用 `HOT_NEWS_REQUIRE_DESCRIPTION` 控制是否要求抓到可用描述才导入。为避免版权风险，不会整篇复制第三方原文。
